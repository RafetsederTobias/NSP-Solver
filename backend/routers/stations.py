...
async def get_stations(db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        select(StationModel)
        .options(selectinload(StationModel.skill_relations))
        .order_by(StationModel.id)
    )
    return [
        {"id": s.id, "name": s.name, "maxAssignments": s.maxAssignments,  "skills_needed": [sk.name for sk in s.skill_relations]}
...
async def get_station(station_id: int, db: AsyncSession = Depends(get_db)):
    ...
    return {"id": station.id, "name": station.name, "maxAssignments": station.maxAssignments, "skills_needed": [s.name for s in station.skill_relations]}
...
async def create_station(payload: StationPayload, db: AsyncSession = Depends(get_db)):
    ...
    station = StationModel(name=payload.name, maxAssignments=payload.max_assignments, skill_relations=skills)
    db.add(station)
    await db.commit()
    result = await db.execute(
...
async def update_station(station_id: int, payload: StationPayload, db: AsyncSession = Depends(get_db)):
    ...
    station.maxAssignments = payload.max_assignments
